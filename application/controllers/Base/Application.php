...
class ApplicationController extends Controller_Abstract
{
    ...
    protected function mca()
    {
        $mca = new Mca();
        $mca->incry($this->getRequest());
    }
    ...
    /**
     * 需要top授权的模块
     * 卖家r1授权过期时，不能进入到 需要授权的模块
     */
    public function needTop()
    {
        if ($this->isNeedTop()) {
            $r1exp = \Bootstrap::getShopModel()->queryTrade()->getR1Expired();
            if (strtotime($r1exp) < time()) {
                $loginModel = LoginModel::getInstance();
                $loginModel->setPreuri($this->getRequest()->getRequestUri());
                $loginModel->pushSession();

                $this->redirect(\Bootstrap::getUrl('/vip/auth/index'));
            }
     ...
     * 判断当前模块是否需要授权
     */
    protected function isNeedTop()
    {
        $request = $this->getRequest();

        $model      = strtolower($request->getModuleName());
        ...
        foreach ($modules as $module => $b) {
            if ($b && $module == $model) {
                if ('print' == $module && in_array($controller, ['express', 'config'])) {
                    return false;
                }
    ...
    {
        $isHigh = \Shop\VersionModel::isHighVersion();
        if (!$isHigh) {
